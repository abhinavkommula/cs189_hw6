...
class NeuralNetwork(ABC):
    ...
    def backward(self, target: np.ndarray, out: np.ndarray) -> float:
        ...
        resultingLoss = self.loss(target, out)
        outputGradient = self.loss.backward(target, out)
        
        for l in self.layers[::-1]:
            outputGradient = l.backward(outputGradient)
        
        return resultingLoss
    ...
    def predict(self, X: np.ndarray, Y: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        Y_np = np.array(Y)
        predictedY_np = np.array(predictedY)
        loss = self.backward(Y_np, predictedY_np)
        
        return predictedY, loss
